...
The following stakeholders directly shape the system's goals, since their needs, workflows, and constraints determine what the platform must support and what requirements are prioritized.
...
Scholars that are working on the reconstruction of Buddhist texts will be directly interacting with the desktop application we plan on building. They will be using the application to match fragments together based on metadata, edge information, and other criteria they decide to choose. Scholars will be using the main functionality of this application and therefore are direct stakeholders.
...
* **Other scholars working on different manuscript reconstructions (specifically ones not in Sanskrit):** While they benefit from the fragment filtering and canvas functionality the system was not built to support other languages that aren’t Sanskrit.
* **Collection staff looking to store fragments locally:** If some researchers want a place to store and organize manuscript fragments safely, they could also use the application. Though this is not the intended use of the system it is definitely a way in which someone can use the system.
